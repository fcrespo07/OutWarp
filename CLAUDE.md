...
- [ ] **Login del dashboard mejor que el token de admin.** Hoy es un token único + sesión en memoria (`server/outwarp_server/web_auth.py`). Evaluar y decidir entre: usuario + contraseña (scrypt, ya en el repo) con TOTP opcional, passkeys/WebAuthn, o enlace de login de un solo uso emitido por CLI (`outwarp-server panel-login`); sesiones que sobrevivan a un reinicio del panel. Antes de 1.0 porque cambia la forma de la config del servidor.
...
- [ ] **Interfaz en los 5 idiomas más hablados del mundo, no solo español e inglés.** Por hablantes totales (Ethnologue): **inglés, chino mandarín (simplificado), hindi, español y árabe estándar**. Cubre GUI cliente/servidor (`STR`/`SRV_STR` en `shared.jsx`/`srv-data.jsx`), panel web, TUIs, mensajes de CLI y notificaciones, con selección automática por idioma del sistema y selector manual. Implica: sacar a i18n los strings hoy fijos; **RTL** para árabe (`dir="rtl"`, layouts espejados); fuentes de respaldo, porque Geist no tiene glifos CJK, devanagari ni árabes; en las TUIs, anchura doble de CJK y los límites de los terminales con RTL. Traducciones revisadas por un hablante nativo, no solo automáticas. Sale de "NO bloqueante" ("más idiomas").
...
La lista completa y vinculante está en **"Criterio de 1.0.0"** (sección Licencia, más arriba). Resumen: e2e en CI · kill switch + hostname · renombrar `outwarp-cli` → `outwarp` (hecho) · GUI Linux de primera clase · compatibilidad Omarchy · vitest + guardia de bundle · fallback sin firma retirado · README/wizard honestos sin el banner "not ready" · sin bugs 🔴 abiertos · activar/desactivar clientes desde el dashboard · estudio de handshakes/cortes RDP · pulido UI/UX · login del dashboard sin token · servidor Windows vía Docker · multi-perfil no simultáneo · UI en los 5 idiomas más hablados · auditoría general final · release firmada tras un ciclo sin hotfix. El instalador Windows sin firmar **no** bloquea.
